...
Simplex *findFirstSimplexToModifyLUTPointTest(Point *point, Partition *partition, UserOptions *options)
{
    ...
    PointWithDensity *pointTemp = (PointWithDensity *)malloc(sizeof(PointWithDensity));
    ...
    while (radius < options->PHgridSize)
    {
        ...
        while (k >= 0)
        {
            ...
            while (boxIdTheSame)
            {
                ...
                BarycentricCoordinates *coords = calculateBarycentricCoordinates(simplex, pointTemp);
